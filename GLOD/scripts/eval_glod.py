...
class GaussianLayer(nn.Module):
    def __init__(self, input_dim, n_classes):
        ...
        self.covs = nn.Parameter(0.2+torch.tensor(np.random.exponential(scale=0.5,size=(n_classes, input_dim))).cuda())
        
    def forward(self, x):
        covs = self.covs.unsqueeze(0).expand(x.size(0), self.n_classes, self.input_dim)
        ...
        Z_log = -0.5*torch.sum(torch.log(self.covs), axis=-1) -0.5*self.input_dim*np.log(2*np.pi)
...
# def search_thers(likelihood_in_distribution, level):
#     step = 1
#     val = -1
#     eps = 0.0001
#     while True:
#         TNR = (likelihood_in_distribution>=val).sum()/likelihood_in_distribution.shape[0]
...
if __name__ == '__main__':
    ...
    for net_num in range(1, 6):
        #  Laod network
        checkpoint = torch.load(f'./GLOD_models/{dataset}_GLOD{net_num}.ckpt', map_location=device)['net']
    
        net.load_state_dict(checkpoint)
        
        llr_in_distribution = predict_llr(model=net,
        ...
        for data_idx, ood_loaders in enumerate(ood_datasets):
            ...
            llr_out_distribution = predict_llr(model=net,
    ...
    if dataset == 'svhn':
        ...
        df.to_csv(f'./GLOD_{dataset}_results.csv')
