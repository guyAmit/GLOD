...
class ResNet_SSL(nn.Module):
    ...
    def forward(self, x):
        out = self.resnet.module.penultimate_forward(x)
        out = self.linear(out)
        return out
...
if __name__ == '__main__':
    ...
    if dataset == 'cifar100':
        net = create_Resnet34(num_c=(n_classes+dummay_classes))
    ...
    for net_num in range(1, 6):
        #  Laod network
        checkpoint = torch.load(f'./SSL/{dataset}/{dataset}_resnet{net_num}_ssl.ckpt.pth')
        ...
        for data_idx, ood_loaders in enumerate(ood_datasets):
            ...
            #  predict ssl
            preds_in = predict(in_test_loader, n_classes).cpu()
            preds_ood = predict(ood_test_loader, n_classes).cpu()
            ...
            # auroc
            y_true = np.concatenate((np.zeros(preds_in.size(0)), np.ones(preds_ood.size(0))))
