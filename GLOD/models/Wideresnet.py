...
class NetworkBlock(nn.Module):
    ...
    def _make_layer(self, block, in_planes, out_planes, nb_layers,
                    stride, dropRate):
        layers = []
...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes,
                 widen_factor=1, dropRate=0.0, gaussian_layer=True):
        ...
        if not self.gaussian_layer:
            ...
            self.gaussian_layer = GaussianLayer(
                input_dim=nChannels[3], n_classes=num_classes)
    ...
    def get_clip_covs(self, beta):
        if self.gaussian_layer:
            return self.gaussian_layer.clip_convs()


def get_WideResnet(depth=40, num_classes=10, widen_factor=2,
                   dropRate=0.3, gaussian_layer=True,):
    net = WideResNet(depth=depth, num_classes=num_classes,
                     widen_factor=widen_factor, dropRate=dropRate,
                     gaussian_layer=gaussian_layer)
    return net
